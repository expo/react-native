...
| 100 messages × 10 lines | 32.5ms | — | **20.4ms** | 31.3ms | — |
| one 1,000-line article | 25.8ms | — | **12.8ms** | 19.3ms | — |
...
| | against upstream `<Text>` | against upstream `NativeText` |
| --- | ---: | ---: |
| 1,000 settings rows | **9% faster** | 2% slower |
| 100 messages × 10 lines | **37% faster** | — |
| one 1,000-line article | **50% faster** | — |

The gain grows with how much text each element holds and is smallest in the row
shape, where every row mounts a view either way and the text is one short line.

`NativeText` is the floor of what the current architecture can do — it is the
host component `<Text>` compiles to, with the JS wrapper removed. A bare string
lands within a couple of percent of it in the row shape while keeping the press
handling, accessibility and layout events `NativeText` gives up.
