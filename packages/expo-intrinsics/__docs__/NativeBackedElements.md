...
> **Superseded — and both options are out of scope now.** Expo UI's `Host`
> already declares the boundary, so host context comes from ordinary React
> context; see `PlatformFidelity.md`. Neither option below is being pursued: (A)
> changes React's reconciler and (B) changes shared platform code, and the work
> here stays inside the new elements. Kept because the reasoning explains why
> `Host` is the right seam.
