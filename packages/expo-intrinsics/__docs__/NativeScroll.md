...
| `contentRef`            | ref                                      | —               | The content container itself, to measure rows against — see below. |
...
### Measuring a row: `contentRef`

Pass a ref to `contentRef` and you have the content container — the one ancestor
every row shares that scrolling does not move. A `measureLayout` against it
answers in CONTENT coordinates: where the row sits in the conversation, a number
that no scroll, inset or keyboard changes. That is what you want for anything
that has to survive the view settling, and it is what the chat demo's send
animation aims at.

### `contentShift`, for a measurement you already have

Only when the measurement is in the WINDOW and you cannot retake it — an API you
do not control, a position handed to you from elsewhere.

This view composes its insets NATIVELY, in the frame the keyboard moves, and the
offset it moves doing that is deliberately never reported as a scroll — the
reader did not move, and telling the tree they had would make a keyboard's
arrival look like one.

The cost lands on anything that measures. `measureInWindow`,
`getBoundingClientRect` and `measureLayout` all answer from the shadow tree, so
a row inside this view reports where it would be if none of that composition had
happened. `contentShift` is the difference, and it turns one into the other:

```js
const drawn = measured - shift.y;
```

It changes only when the composition does, so it is a value to hold from the
event rather than a per-frame reading. Measured on a chat whose content was
shorter than its window: a balloon the tree put at 332.7 was drawn at 448.7, and
the difference was the 116-point top inset exactly. Two wrongs had been
cancelling — a transcript pinned to the bottom under a raised keyboard is
displaced by exactly what a docked composer is — so a send animation that flew
between the two was right until the day the content was short.
