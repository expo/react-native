...
export type FormRegistry = {
  ...
  /*
   * The form's own `autocorrect`, for the controls inside it to inherit.
   *
   * On the form registry rather than in a context of its own because it is the
   * FORM OWNER a control consults, not its nearest ancestor: `autocorrect` does
   * not inherit down the tree the way `spellcheck` does (§6.8.8), and a control
   * finds its form owner through exactly this object. Null where the form says
   * nothing, which leaves the control's own default intact.
   */
  readonly autocorrect?: 'on' | 'off' | boolean | null,
  /*
