 ...
 */
/**
 * How long the pill takes to give back the lines a sent message was using.
 *
 * The native app spends nothing here — it collapses in one frame, under the
 * balloon — so this is not a measurement of it. It is the shortest ease that
 * reads as the field letting go rather than as a jump, against a flight that
 * takes about half a second to leave.
 */
const FIELD_SHRINK = 180;

/**
 * How long the pill waits before it starts giving the height back.
 *
 * Long enough for the send's own commits to land — the row, the handoff, the
 * receipt's space — so the ease begins in a commit of its own. The balloon is
 * clear of the bar by then, so the wait is not visible as a pause.
 */
...
/*
 * The audio-message glyph's colour and bars — see `AUDIO_GLYPH` below.
 *
 * The PLACEHOLDER's colour, which is a translucent label colour
 * (`UIColor.placeholderTextColor`) and not a grey. Both of the things in an
 * empty field say the same thing — "nothing here yet" — and the platform
 * colours them the same way, so the glyph follows the placeholder into dark
 * mode, into an accessibility contrast setting, and over whatever is behind the
 * field rather than sitting flat on it. A balloon passing underneath tints it,
 * which a solid grey cannot do.
 *
 * Asked at runtime the native tint reads `rgba(180, 184, 191, 1)` in light and
 * `rgba(74, 75, 77, 1)` in dark — the composited answer, which is what an
 * opaque copy of it was fitted to.
 */
...
/*
 * The bars, in points, and they are not an SF Symbol.
 *
 * iOS 27 replaced the composer's dictation microphone with an audio-message
 * button, and it is `AudioMessageEntryViewButton` — an asset in ChatKit's own
 * catalogue, not something `system:` can name. Rendered out of that catalogue
 * it is five symmetric bars, 8 / 16 / 32 / 16 / 8 tall, 4 wide, 8 apart; on a
 * device it draws at half that, which is the 18.00 x 15.67 of ink measured off
 * a screenshot of the real app.
 *
 * Symmetry is the tell that it is not a symbol: every waveform in the SF
 * catalogue is lopsided (`waveform` is 4 / 10.67 / 17.33 / 8.67 / 14 / 5.33
 * over six bars), which is what ruled them out one by one.
 *
 * Drawn rather than shipped, because the asset is Apple's: five rounded rects
 * cost nothing to lay out and the element is built once at module scope, so a
 * composer that re-renders on every keystroke re-uses it rather than rebuilding
 * it.
 */
...
const SEND_W = 38;
/*
 * The send button's clearance from the pill: SIX above and below, and SIX AND A
 * HALF at the trailing edge.
 *
 * Measured against the native app on the same simulator with a draft typed into
 * both: its button is 38.00 x 28.00 — ours to the third of a point — with its
 * right edge at 379.67 against a pill edge whose stroke runs 385.67 to 386.33,
 * so the two readings bracket the clearance at 6.33 and 6.67.
 *
 * The vertical six is what makes the height: a forty-point pill less twice six
 * is twenty-eight, which is the height measured.
 *
 * This number is also the field's WRAPPING, which is why it is worth a third of
 * a point. The text column ends where this button begins, and the native
 * composer's column and the native balloon's are the same width: the same
 * hundred and fifty narrow glyphs fill 250.00 of the field and 249.67 of the
 * balloon, so a line the writer sees is a line the balloon has. See
 * `BALLOON_MAX_WIDTH` in `ChatScreen`, which is that same column.
 */
const SEND_INSET = 6;
 ...
 * `align-self: center` so that it behaves like the send button when the field
...
/**
 * And the trailing one, which is the SAME — because the pill's right edge and a
 * sent balloon's right edge are the same line.
 *
 * Measured off a clip of the native chat sent from a device, 393-point window:
 * its pill ends at 376.7 and its balloon rests at 376.7, both 16.3 from the
 * screen. That alignment is what makes a send read as the field becoming the
 * balloon — the one edge that does not move through the whole animation. Twelve
 * here left the pill three and a half points outside the balloon born over it,
 * reported from a device as the bubble being "slightly wider than the text
 * area".
 */
...
export default function Composer({
  ...
  onFieldSize,
  /**
   * A handle on the pill, for the one question a size cannot answer: where the
   * field IS. The flight layer and this are measured against each other — see
   * the `ref` on the pill below, and `flightLayer`.
   */
  fieldRef,
  /**
   * Text this field is still SHOWING for a message it has already handed over.
   *
   * A send cannot put the balloon on screen in the same frame the way the
   * native app does — the flight waits for the transcript to be still — so the
   * field keeps drawing what it sent until the balloon says it is visible. The
   * draft is empty from the tap; this is pixels, not state, which is why the
   * send button goes at once and typing is ignored while it stands.
   */
  handoff,
  actions = [],
  /**
  ...
  /*
   * The send button stands while the field is holding a sent message, and that
   * is about WIDTH rather than about the button.
   *
   * The field is as wide as the button leaves it. Take the button away while
   * the text is still being drawn and the same text rewraps onto fewer lines —
   * measured: a two-line draft became one line for the three frames before the
   * balloon appeared, which is the field visibly rewriting a message the reader
   * has already sent. Holding it keeps those frames identical to the last frame
   * before the tap. It cannot be pressed again: the draft behind it is empty.
   */
  const hasDraft = value.trim() !== '' || handoff != null;
  /* What the field draws: its own value, or the message it is still holding. */
  ...
  /*
   * The field EASES back to one line after a send, and the balloon is why it
   * can.
   *
   * The native app does not animate this in either direction — measured, frame
   * by frame, it collapses in one — because at that frame its balloon is a box
   * sitting exactly over the field, and the snap happens underneath something.
   * Ours leaves from the bar's top edge instead, so the same frame is in the
   * open: a two-line field becoming one while the reader is looking at it.
   *
   * A LAYOUT TRANSITION, not an animated height. `CSSLayoutTransitions` lays
   * the tree out once at the destination and glides the mounted metrics between
   * two real layouts — one clone, one scratch Yoga pass, and no commit per
   * frame. Driving a height from JavaScript instead is the per-frame relayout
   * this fork's engine was built to replace, and the field's height is also the
   * BAR's, which UIKit animates inside the keyboard's transition: two clocks on
   * one edge, which is the jitter recorded below.
   *
   * Declared always and timed by DIRECTION. A transition declared only at the
   * send never starts — the engine's trace shows no height transition beginning
   * at all — and one declared permanently eases the GROWTH too, which reads as
   * the field wobbling under the typing. Zero milliseconds is a snap, so
   * growing keeps the single frame it has always had.
   */
  /*
   * The pill HOLDS its height through the send's own commits, and eases in a
   * quiet one.
   *
   * The send lands several commits in a handful of frames — the row, the
   * handoff's release, the receipt's space — and a transition started inside
   * that burst is re-presented by the commits that follow it: the engine's
   * trace shows `start t=512 h 60.712px->40.712px` and a flight registered with
   * the right metrics, and the pixels snap anyway. Waiting for the burst to
   * pass leaves the height change alone in its commit, which is the state the
   * engine is built for.
   */
  ...
  /*
   * LATCHED for the length of the ease, and that is not bookkeeping.
   *
   * The duration has to be in the same commit as the height — a transition runs
   * on the after-change style — so it is computed here rather than set from an
   * effect a render late. But the comparison that produces it is true for one
   * render only, and the next commit carried `0ms` again: the engine took that
   * as the transition's new duration and ended it. Measured with the pill
   * tinted while the flag stood — red for two frames of a 600ms ease, and the
   * height snapping in one. So the flag stands until the ease is over.
   */
    ...
    <div style={styles.row} pointerEvents="box-none">
      ...
      */}
      <div
        collapsable={false}
        /*
         * The pill answers "where is the field", and the flight is drawn INSIDE
         * it — so the answer it has to give is its own box: an x in the window
         * to place the destination against, and a height to hang the balloon
         * from. A view inside the accessory reports the y the bar was LAID OUT
         * at, which is where it rests with the keyboard down and 328 points
         * from where UIKit draws it; the x is the same either way, because the
         * bar spans the window at both.
         */
        ref={fieldRef}
        style={[
          styles.glass,
          shrinking ? styles.glassShrinking : styles.glassSettled,
          // The field's height, or the one being given back — see `held`.
          {height: pillHeight},
        ]}
        onLayout={event => {
        ...
        <textarea
          ...
          //
          // Ignored while the field is holding a sent message: the draft is
          // already empty, and a keystroke landing in those few frames would
          // otherwise arrive as the sent text plus a letter.
          onInput={event => {
            if (handoff != null) {
              return;
            }
        ...
        />
        {/*
          The AUDIO-MESSAGE button, which is what the platform puts here when
          the field is empty — and the field is empty most of the time.

          It used to be a dictation microphone and iOS 27 changed it: the glyph
          is now ChatKit's `AudioMessageEntryViewButton`, five symmetric bars.
          See `AUDIO_BARS` for the measurement and for why no SF Symbol matches
          it.

          DECORATION, said out loud. The platform's is a real button that starts
          an audio message, and there is no API here to record one, so a
          `<button>` would be a control that looks live and does nothing — worse
          than not having one. `aria-hidden` keeps it out of the accessibility
        ...
        */}
        ...
        {hasDraft && (
          <button
            ...
            onClick={() => {
              // Held: the draft behind this button is already empty, and the
              // send it would repeat is the one in flight.
              if (handoff != null) {
                return;
              }
              onSend(value.trim());
...
export function ComposerBar({
  children,
  /**
   * What a send throws, drawn between the bar's own chrome and the field — see
   * `flightLayer`.
   */
  overlay,
  /** The layer it flies in, so its owner can ask where it is. */
  flightLayerRef,
  /**
   * The bar itself, which is the ANCESTOR both the layer and the field are
   * measured against — see `rememberFlightFrame` in `ChatScreen`.
   */
  boxRef,
}: {
  children: React.Node,
  ...
  return (
    <NativeKeyboardAccessory
      ...
      {/*
        FIRST, so everything the bar paints is drawn OVER it: its blur, the
        glass container, the field's own glass, and the field's content.

        A send's balloon is born as the field's box and the platform draws it
        UNDER the composer — measured frame by frame on the same simulator, the
        balloon's bottom is cut off by the pill's top edge as it leaves and what
        is still behind the field shows through it. That is the emerging the bar
        exists to give it, and drawing the balloon over the bar instead reads as
        a balloon popping on top of the composer.

        What the balloon then reads as is a question about the MATERIALS, not
        about this order: see `BAR_MATERIAL_STRENGTH` and `FIELD_GLASS_KEYWORD`.

        `collapsable={false}` because this box is asked where it is before it
        has anything in it, and Fabric flattens an empty view that paints
        nothing — a flattened view has no ref to measure, and the send then drew
        the balloon at the bar's own origin.
      */}
      <div
        ref={flightLayerRef}
        collapsable={false}
        style={styles.flightLayer}>
        {overlay}
      </div>
      {/*
...
const styles = StyleSheet.create({
  ...
  /*
   * The layer a send's balloon flies in, hanging from the BOX's bottom edge.
   *
   * The BOTTOM, and that is a correction: a bar sits on the keyboard, so its
   * bottom is the edge that does not move. When the pill collapses from two
   * lines to one the bar gets shorter from the TOP, and a layer pinned up there
   * carries whatever is flying in it down by the points the pill gave back —
   * measured on a two-line send, the balloon landed twenty low and jumped up
   * when the row took over.
   *
   * Zero-height, so it takes no space in the box; `pointerEvents: 'none'`, so
   * the drag that dismisses the keyboard still reaches the bar underneath.
   */
  flightLayer: {
    position: 'absolute',
    bottom: 0,
  ...
  glass: {
    ...
    borderRadius: LINE / 2,
    /*
     * The pill's height is a LAYOUT TRANSITION — see the `shrinking` note where
     * it is timed. Declared here so the property is on both faces of the change.
     */
  ...
  field: {
     ...
     */
    /*
     * SIXTEEN, read again with the two composers a frame apart and the same
     * draft typed into both: the native caret stands at 96.33 and ours stood at
     * 95.33, off pill edges that are the same to a third of a point. The
     * earlier reading above put it at fifteen; this one compares like with like
     * — same simulator, same glyphs, caret against caret — so it is the one the
     * field follows.
     *
     * Material's field insets its text by 16 as well, for its own reasons.
     */
    paddingLeft: 16,
    /*
     * FIVE, and the number is chosen so that the field's text column is the
     * BALLOON's.
     *
     * The platform keeps those two the same width: a hundred and fifty narrow
     * glyphs fill 250.00 of the native field and 249.67 of a native balloon, so
     * the line breaks a writer sees while typing are the line breaks the
     * message is sent with, and nothing has to re-wrap when it leaves. Ours were
     * a character apart — the field broke first — which is a word jumping
     * between lines at the moment of the send.
     *
     * The same probe brackets the native field's own trailing inset between 2.6
     * and 6.2; five is inside it, and five is what makes our column
     * 252.67 — `BALLOON_MAX_WIDTH` less twice `BUBBLE_PADDING`, to the point.
     * `WrapCheck` is the test that keeps them together.
     */
    paddingRight: 5,
    /*
  ...
  /*
   * The pill's two speeds.
   *
   * A transition runs on the AFTER-change style's declaration
   * (css-transitions-1 §3), so a duration that appears only once the field has
   * shrunk describes a change that has already happened and never starts. Both
   * faces are declared; the growth's is zero, which is the single frame the
   * field has always grown in.
   */
   ...
   */
  /*
   * The audio glyph's row, against the pill's bottom line.
   *
   * `flex-end` rather than centred for the same reason the send button is: when
   * the field grows to three lines both stay on the last line rather than
   * drifting to the middle of the pill.
   */
  audio: {
    display: 'flex',
    flexDirection: 'row',
  ...
  audioBar: {
    ...
    /* Rounded ends, which the asset's bars have: half the bar's own width. */
...
/*
 * Built ONCE, after the sheet it reads.
 *
 * The glyph never changes, so the composer re-uses this element rather than
 * making five boxes again on every keystroke — and at module scope it has to
 * come after `styles`, or it would read the sheet before it exists.
 */
const AUDIO_GLYPH = (
