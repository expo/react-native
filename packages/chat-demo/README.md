...
- **CSS ANIMATIONS do not do layout properties, and that is the piece missing
  ...
  engine.
- **The send flight is CLIPPED by the transcript, and the platform's is not.**

  NOTE (2026-09-13): **done — the balloon is drawn inside the bar.** What a send
  throws is a COPY, rendered in a flight layer inside the accessory
  (`ComposerBar`'s `overlay`), born as the field's own box and passing over the
  composer while the row it is going to waits invisible at the destination.
  `measureLayout` between the field and that layer is what relates the two
  coordinate spaces: `measureInWindow` inside the accessory reports where the
  bar was LAID OUT — 817 for a bar being drawn at 442 — and no arithmetic on a
  wrong number fixes it. The destination is an animated value the row rewrites
  every frame it moves and the spring runs on a 0 → 1 FRACTION of it, so the
  transcript can still be scrolling while the balloon is in the air, which is
  what the platform does and what this used to do one after the other.

  Two things had to be true for the landing, both measured on 60 fps captures.
  The copy is handed back when it is within half a point of the row, NOT when
  its spring reports finished: on a 0 → 1 fraction the default rest threshold is
  six hundredths of a point, so "finished" arrived 600 ms after the balloon was
  visually still — and an animated view goes back to the props React last
  rendered the moment its animation ends, which put the balloon at the composer
  again for the twelve frames the arrival's own commit took to paint and read as
  the send happening twice. And the copy is hidden before it is unmounted, so no
  ordering of those two can be seen. Flight, measured end to end: 667 ms.

  The measurement that started this, both apps' balloons tracked as connected
...
- **iOS 27's composer has no microphone.** The dictation button is gone, and
  what sits in an empty field is an audio-message button whose glyph is not an
  SF Symbol at all — it is ChatKit's own `AudioMessageEntryViewButton` asset,
  five SYMMETRIC bars. That symmetry is what ended the search: every waveform in
  the SF catalogue is lopsided. The iOS 27 runtime lives in a cryptex and the
  framework has no Mach-O left to read, but its `Assets.car` is still a file and
  `assetutil --info` still reads it, which is where the shape and its
  proportions came from. Drawn here as five rounded rects built once at module
  scope — see `AUDIO_BARS` in `Composer.js`.
- **A position measured inside the transcript is a LAYOUT position, and the
  flight is drawn in a different space.** Reported from a device as multiline
  sends being glitchy, and the reporter's guess — that the balloon is aimed at
  where the row would be if the composer stayed tall — was the right shape and
  the wrong cause.

  `<native:scroll>` composes its insets natively and does not report the offset
  it moves doing it, so `measureInWindow` on a row answers where the row would
  be with none of that composition applied. The composer's own frame is a
  layout number too, and the flight is the DIFFERENCE of the two — which is
  right exactly when both are displaced by the same amount. A transcript pinned
  to the bottom under a raised keyboard is, which is why this survived so long.
  Measured in a four-message conversation: the row reported 332.7 and was drawn
  at 448.7, so the balloon flew 116 points too high and the row appeared under
  it as it landed.

  The element now publishes `contentShift` — the part of the offset the tree was
  never told about — and any caller holding a window measurement can convert it.
  See `NativeScroll.md`. The send does not use it: it stopped measuring in the
  window at all, which is the bullet below.
- **A send is aimed at a computed END STATE, not at a measured position.** Three
  things move a row while its balloon is in the air — the list scrolling to its
  end, the pill collapsing back to one line under it, and the receipt handing
  over — so a flight aimed by measurement is aimed at whichever of them has
  happened so far. Measured on a two-line send: the copy came to rest eleven
  points below the row it was standing in for, and the row appeared over it as
  the copy went. The reporter's own diagnosis was the way out: *"we know what
  size the pill will shrink to, thus we can calculate what the new inset and
  offset of the scroll view"* will be.

  So the row reports its box in the scroll view's CONTENT coordinates —
  `measureLayout` against the content container, which is an ancestor, so
  nothing about scrolling, insets or the keyboard is in the number — and the
  composer works out where that content will be RESTING: the pill's collapse is
  known, so the bar's height is, so the bottom inset is; the content's bottom is
  this row's own bottom plus the container's padding; and a scroll view at its
  end rests at `content - height + inset`, clamped by its top inset the way
  `UIScrollView` clamps it. One number, once, at the tap — and the per-frame
  chase of a moving destination is gone with it.

  The receipt's own lift is deliberately NOT in that number. It happens in the
  commit that reveals the row, and from then on the row is there to ride it, as
  the native one does — measured off a clip of it, the balloon rises 18 points
  over 16 frames while its receipt takes over. Measured after: our balloon comes
  to rest on the computed number to a point, the row is revealed exactly there,
  and the column lifts fourteen points under it over three hundred milliseconds.
- **The field's text column IS the balloon's, and the flying copy is laid out
  once.** A sent message is given the row's width at the tap and keeps it for
  the whole throw — nothing reflows in the air — which is only invisible if the
  line breaks it flies with are the ones the writer was looking at. The platform
  arranges exactly that: a hundred and fifty narrow glyphs fill 250.00 points of
  the native composer and 249.67 of a native balloon, so the two wrap the same
  text the same way and the question of "which wrapping does the flight use"
  never arises. Ours were a character apart — the field broke first — because
  the send button sat 3.7 points too far inside the pill and the text started a
  point too far left. Both are measured against the native composer in the same
  frame now, and `WrapCheck` asserts the two columns break at the same glyph.

  The other half of the same rule is the copy's own box: it takes the row's
  width EXACTLY, unrounded. The drawn surface is snapped inward to the pixel
  grid — that is what keeps it inside the field's own edge at takeoff — but a
  box that lays text out has nothing to spare, and a third of a point off it
  cost a four-letter message its last letter: "You" flew as "Yo" over "u" for
  the whole flight and snapped back to one line when the row took over.
- **`Delivered` becoming `Read` is a FADE, not a swap.** Measured off a 60 fps
  capture of the native chat sent by the reader, ink counted per frame: the old
  word fades over seven frames, the line is empty for ten, and the new one fades
  in over eleven at its full width — no growth, no movement, the trailing edge
  fixed. Ours cut between the two strings in a single frame. The numbers are in
  `receiptTiming.js` beside the handover's, which is the same three-part shape
  on a longer clock.
- **The field is handed what it is already drawing, not what was sent.** The
  message is trimmed and the field's text is not, so a send used to ask a native
  text view to change its text — trimmed — and then empty it, two writes a few
  frames apart. The second one did not always land, and the field kept the
  trimmed message after sending it: reported from a device, pinned by
  `DraftCheck` (type trailing spaces, send, the field must be empty), which
  fails on the old shape. Messages avoids the whole question by not trimming;
  this keeps the trim and moves it out of the handoff, so the words the reader
  is looking at never change under them and there is one write to make, at the
  end.
- **A send during a drag does NOTHING.** The native chat enforces it at the
  TOUCH: with a finger on the list its send button does not respond at all.
  Ours still lights up and the tap does nothing, which is one step later —
  taking the button out of reach needs the drag flag in state, and a commit at
  the start of every drag re-renders the transcript's rows, which is what the
  suite's two interactive keyboard dismissals went red on. This held the send
  instead — the draft was taken at
  the tap and the insert waited for the finger to leave — and the report says
  why that is worse than nothing: "the text area appears to clear but no bubble
  sends, and when I lift the scrolling finger the message sends and the text
  area's contents are replaced with the new message's text". The second half is
  the handoff, which keeps the field drawing the words until the balloon is on
  screen; it is right for a send that happens and wrong for one that waited.
