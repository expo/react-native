...
final class ReceiptCheck: DemoCase {
  ...
  func testTheReceiptSitsAgainstTheBalloon() throws {
    ...
    /*
     * `Read`, and not `Delivered`, which is a RACE.
     *
     * The words change under a message that is already wearing them — the demo
     * fades `Delivered` out, leaves the line empty for a sixth of a second and
     * fades `Read` in — so a poll that matches the first one can find it, and
     * then ask for its frame after it has become the second. XCUITest answers
     * that with "failed to get matching snapshot" and the case fails on the
     * element it was standing on. `Read` is where this conversation settles and
     * it never changes again; anything measured on it is measured on a line
     * that is done moving.
     */
  ...
  func testTheReceiptPushesTheTranscriptClearOfTheComposer() throws {
    ...
    // `Read` only — see the case above for why matching `Delivered` is a race.
    var receipt: XCUIElement?
    let deadline = Date().addingTimeInterval(10)
    while Date() < deadline && receipt == nil {
      let any = app.descendants(matching: .any)
