...
final class DraftCheck: DemoCase {
  ...
  /**
   A send empties the field, whatever it had to change about the text.

   The message is TRIMMED and the field is not: type trailing spaces and the
   words that go into the balloon are not the words the field is holding. That
   difference is the bug this pins — reported from a device as "the text area
   retains the trimmed text even after sending it". The field keeps drawing the
   message until the balloon appears (see `handoff`), and when what it was asked
   to draw differed from what it already had, the send became TWO writes to a
   native text view a few frames apart: the trimmed text, then the empty draft.
   The second one did not always land.

   Trailing spaces are the cheapest way to make the two differ, and a device
   makes them by default — the space bar's double-tap is a full stop and a
   space. The assertion is only that the field is empty afterwards.
   */
  func testASendWithTrimmableTextLeavesTheFieldEmpty() throws {
    ...
    app.typeText("Trim me   ")
    let send = app.buttons["Send"]
    XCTAssertTrue(send.waitForExistence(timeout: 8), "no send button with a draft")
    send.tap()

    // The balloon has to have arrived before the field can be asked: the field
    // is deliberately still drawing the message until it does.
    XCTAssertTrue(
