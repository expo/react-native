...
/**
 * Whether a pseudo-class is one the runtime tracks as live interaction —
 * `:root` and `:disabled` are read off the element itself, not from touch.
 */
...
export type Matcher = {
  ...
  // position (`.group:hover .x`), which is the only reason an element whose
  // own styling never changes still has to track state.
...
export function createMatcher(sheets: Array<Stylesheet>): Matcher {
  ...
  for (const sheet of sheets) {
    ...
    for (const rule of sheet.rules) {
      for (const selector of rule.selectors) {
        ...
        // is not the subject, so it is not indexed as a candidate and would
        // never learn that anything depends on whether it is hovered — it has
        // to be told. Collect the classes read that way; an element carrying
        // one of them tracks interaction even when its own rules do not.
  ...
  return {
    ...
    anyStateDependent,
    // Classes some rule reads interaction state from in an ancestor position.
