        ...
        {entries.length > 0 ? (
          ...
          <View
            ...
            {entries.map(entry => (
              <View
                key={entry.id}
                style={[StyleSheet.absoluteFill, styles.overlayText]}
                pointerEvents="box-none"
...
export function useTopLayer(
  ...
  /*
   * The custom properties in scope where the overlay was WRITTEN.
   *
   * React context follows the element tree, and this content is handed to a
   * host mounted at the app root — so inside the top layer it sees the HOST's
   * scope, not the caller's, and every `var()` it uses resolves to nothing.
   * That is not cosmetic: an unresolved `var(--border-width)` reaches Android
   * as the literal string and `borderWidth`'s float setter throws
   * ClassCastException, which red-boxed the whole screen the moment a popover
   * opened.
   *
   * Carrying the scope over is what the web already does by not moving the
   * element at all: its top layer is a paint order, and inheritance still runs
   * down the DOM tree it was written in.
   */
  const scope = useContext(VarScopeContext);
  ...
  useEffect(() => {
    ...
    if (open) {
      api.present(id, mode, scoped);
    } else {
      api.dismiss(id);
    }
    return () => api.dismiss(id);
  }, [api, id, open, mode, scoped]);
...
const styles = StyleSheet.create({
  ...
  /*
   * The colour overlay content inherits.
   *
   * On the web the top layer is a PAINT ORDER, not a tree change: a dialog's
   * text still inherits from the element it was written inside. Here the
   * overlay is genuinely re-parented to the app root, so that inheritance is
   * cut and its text fell to React Native's default black — which on a dark
   * popover is text that is not there. A dialog heading and a popover's prose
   * were both reported invisible in dark mode for exactly this reason.
   *
   * The platform's own label colour is the honest replacement: it follows the
   * scheme the way every other native surface does, and an author colour still
   * beats it. Restoring the WRITING SITE's cascade would be better still and
   * is a bigger change — the overlay would have to carry its ancestors' text
   * context across the re-parent.
   */
  overlayText: {
