...
function TextAttributes() {
  ...
  return (
    <Screen
      ...
      <Case
        title="disabled"
        note="Inert, and drawn that way — the text greys from each platform's own disabled colour.">
...
function Checkables() {
  ...
  return (
    <Screen
      intro={
        'Checkboxes and radios. Deviation: iOS has no checkbox, so a form boolean ' +
      ...
      <Case
        ...
        {[
          ...
          <View key={value} style={CONTROL_ROW}>
            ...
            />
            <Text style={CONTROL_LABEL}>{text}</Text>
          </View>
        ))}
      </Case>
    </Screen>
...
function SelectControls() {
  ...
  return (
    <Screen
      intro={
        '<select> with <option> children. The options are handed to the control as ' +
        'one list, so an <option> never mounts. Deviation: a pop-up UIMenu on ' +
        'iOS and a modal list on Android, not Safari’s wheel.'
      }>
  ...
  return (
    <Screen
      ...
      <Case
        ...
        note="Empty, the platforms differ: iOS shows today (a compact picker always shows a date), Android a yyyy-mm-dd placeholder. Both report an empty value until you pick one."
      ...
      <Case
        title='type="file"'
        note="Deviation: the system document picker, and change reports a URI with name, size and type rather than the bytes — a picked video would not fit in memory."
        readout={
          file == null
            ? 'no file chosen'
...
function Buttons() {
  ...
  return (
    <Screen
      intro={
        'A <button> in a form submits it by default — type defaults to "submit", ' +
        'the rule ported web code relies on. Outside a form it is inert.'
      }>
...
function WholeForm() {
  ...
  return (
    <Screen
      intro={
        'Every control registers with the form, so submit collects a FormData from ' +
  ...
  return (
    <Screen
      ...
      <Case
        ...
        note="HTML's other association, needing no id. The label is inline, so the control sits on its line, and all of the label's text becomes the name.">
