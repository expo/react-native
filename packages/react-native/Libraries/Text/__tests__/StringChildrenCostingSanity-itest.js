...
function measuredHeight(children: React.Node): number {
  ...
  Fantom.runTask(() => {
    root.render(
      <View ref={ref} collapsable={false} style={{fontSize: FONT_SIZE}}>
        {children}
      </View>,
    );
  });
...
test('a message body under pre-line renders ALL ten lines', () => {
  ...
  const h = measuredHeight(
    ...
    <View style={{whiteSpace: 'pre-line', fontSize: FONT_SIZE}}>
      {body(10)}
    </View>,
  );
...
test('the two message-list tiers render IDENTICAL content: Text body height equals bare body height', () => {
  ...
  const textBody = measuredHeight(
    <Text style={{fontSize: FONT_SIZE}}>{body(10)}</Text>,
  );
...
test('the two article tiers render IDENTICAL content: 1,000 lines each', () => {
  ...
  Fantom.runTask(() => {
    root.render(
      <ScrollView>
        {/*
         * Both tiers are pinned to the same size, because they no longer
         * default to the same one: a <Text> is an inheritance boundary and
         * keeps React Native's 14, while text written as element children
         * takes the document root — the platform's body size. Comparing the
         * two tiers' heights is only a statement about layout WORK if the
         * type is identical; unpinned, this test would be measuring the
         * difference between 14pt and 17pt and calling it a costing result.
         */}
        <Text ref={textRef} style={{fontSize: FONT_SIZE}}>
          {body(1000)}
        </Text>
        {/* $FlowFixMe[incompatible-type] whiteSpace is a new style key */}
        <View
          ref={bareRef}
          style={{whiteSpace: 'pre-line', fontSize: FONT_SIZE}}>
          {body(1000)}
        </View>
      </ScrollView>,
    );
