...
struct ExpoScrollEvent : public EventPayload {
  ...
  /*
   * The part of the offset the SHADOW TREE was never told about.
   *
   * This view composes its insets natively, in the frame the keyboard moves,
   * and the offset adjustments that composition makes are deliberately not
   * reported as scrolls — the reader did not move, and telling the tree they
   * had would make a keyboard's arrival look like one. The cost is that every
   * position measured from the tree is a LAYOUT position: `measureInWindow` on
   * a row answers where the row would be if none of that composition had
   * happened.
   *
   * So this is the difference, and it is what turns one into the other:
   *
   *     drawn = measured - contentShift
   *
   * It changes only when the composition does — a keyboard, a safe area, a
   * corner — which is when this event fires, so a caller can hold it rather
   * than ask per frame. Measured on a chat whose content was shorter than its
   * window: a balloon the tree put at 332.7 was drawn at 448.7, and the
   * difference was the 116-point top inset exactly.
   */
  Point contentShift{};
