...
/*
 * A comma-separated CSS list, split at the TOP level only.
 *
 * Depth matters because the values in these lists are functions with commas
 * of their own: `steps(4, jump-end)` and `cubic-bezier(.1, .2, .3, .4)` are
 * one timing function each, not two and four. Splitting them blindly produced
 * fragments that parse as nothing, and an unparseable timing function falls
 * back to the default — so every functional easing in a list silently became
 * `ease`, with no error anywhere to say so.
 *
 * The same function splits a function's own arguments, where the depth
 * counter simply never leaves zero.
 */
  ...
  if (value.rfind("steps", 0) == 0) {
    ...
    if (open != std::string::npos && close != std::string::npos &&
      ...
      if (!args.empty()) {
        ...
        // css-easing-1 §2.3, all four jump positions. `start`/`jump-start`
        // jump at each interval's beginning and `end`/`jump-end` (the default)
        // at its end; `jump-none` and `jump-both` also change how many jumps
        // span the interval, which is why the position is carried as an enum.
