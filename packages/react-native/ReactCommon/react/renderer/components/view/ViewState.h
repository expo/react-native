...
class ViewState final {
 public:
  struct TextRun {
    ...
    /*
     * The anonymous box's tag for content runs; 0 for runs that opt out of
     * the measure->mount layout handoff (outside list markers share their
     * box's tag with the content run and would collide, and their layouts
     * are trivial). Keys the handoff registry (run-layout-reuse-plan.md).
     */
    Tag runTag{0};
