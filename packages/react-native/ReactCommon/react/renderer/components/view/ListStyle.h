 ...
 *
 * The §6.4 scripts need no new machinery — `roman` below is already an
 * additive loop and they are additive tables in the same shape. What they need
 * is a way to CHECK the tables: about 140 glyphs across the four styles, where
 * one transposed symbol is silently wrong output rather than a failure. The
 * browser oracle cannot settle it, because generated `::marker` text is not
 * readable from script — `getComputedStyle(el, '::marker').content` gives the
 * specified value, not the resolved string. Adding the tables is half an
 * hour; finding ground truth for them is the actual task.
 *
 * LOWER PRIORITY, and deferred rather than blocked. Nothing here is
 * fundamental: the additive machinery exists, the tables are published, and a
 * verification channel is a day's work rather than a research problem. It is
 * simply not wanted yet. `@counter-style` is not modelled either: an
