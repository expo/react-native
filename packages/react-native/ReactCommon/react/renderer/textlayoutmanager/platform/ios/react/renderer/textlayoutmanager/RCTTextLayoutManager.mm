...
/*
 * `NSBaselineOffsetAttributeName` is NOT applied here, and used to be.
 *
 * This class carried a `drawGlyphsForGlyphRange:atPoint:` override that
 * translated the context by the offset before drawing, on the stated premise
 * that "TextKit 1's `NSLayoutManager` does not honour the attribute — probed: a
 * 0.3em offset moved nothing". That premise is wrong. Probed again, against a
 * bare `NSLayoutManager` with the offset on ONE character of two:
 *
 *     glyph A (offset +4) y=13.00 | glyph B (no offset) y=17.00 | difference 4.00
 *
 * TextKit honours it in LAYOUT, per range. So the drawing pass was applying it
 * a second time, and every baseline offset moved the ink twice.
 *
 * What that cost: `RCTApplyBaselineOffset` centres a run's glyphs in a line
 * taller than the font by adding half the leading as a baseline offset. Doubled,
 * the text sat that much too high in its line — reported from the device as text
 * too high in Astryx's ported components and in shadcn's buttons and tabs, and
 * measured in a 20pt line at 12pt: the baseline landed at 11.14pt where CSS puts
 * it at 14.54. Removing the second application puts it at 14.16.
 *
 * The symbolic list markers state their x-height centring through the same
 * attribute and are unaffected: the probe above is exactly their case — one
 * range offset within a line — and TextKit places it correctly on its own.
 */
                           ...
                           layoutConstraints:(LayoutConstraints)layoutConstraints
{
  ...
  NSTextStorage *textStorage = [self _textStorageAndLayoutManagerWithAttributesString:attributedString
                                                                  paragraphAttributes:paragraphAttributes
                                                                                 size:maximumSize
                                                                           exclusions:layoutConstraints.floatExclusions];
                         ...
                         layoutConstraints:(LayoutConstraints)layoutConstraints
{
  ...
  if (layoutContext.runTag != 0 && nsAttributedString.length != 0) {
    ...
    NSTextStorage *textStorage =
        [self _textStorageAndLayoutManagerWithAttributesString:nsAttributedString
                                          paragraphAttributes:paragraphAttributes
                                                         size:maximumSize
                                                   exclusions:layoutConstraints.floatExclusions];
    ...
    // Nor a run beside a float: the cache is keyed on the container WIDTH, and
    // two runs of the same string at the same width lay out differently when
    // one of them has a float eating into its lines.
    if (!hasAttachments && layoutConstraints.floatExclusions.empty()) {
...
- (std::vector<facebook::react::Rect>)
    getFragmentRectsWithAttributedString:(AttributedString)attributedString
                     paragraphAttributes:(ParagraphAttributes)paragraphAttributes
                                    size:(CGSize)size
                              exclusions:(const std::vector<FloatExclusion> &)exclusions
{
...
/*
 * Float exclusions become `exclusionPaths` on the text container: TextKit
 * shortens exactly the line fragments a path overlaps and leaves the rest
 * full width, which is what CSS2 §9.5 asks for and is why this is a rect per
 * band rather than a smaller container.
 *
 * No unit conversion, and that is not an omission: the renderer's
 * device-independent unit IS the iOS point, which is what a text container
 * measures in. The Android side of this feature has to convert, because a
 * `Layout` there measures in physical pixels.
 */
- (NSTextStorage *)_textStorageAndLayoutManagerWithAttributesString:(NSAttributedString *)attributedString
                                                paragraphAttributes:(ParagraphAttributes)paragraphAttributes
                                                               size:(CGSize)size
                                                         exclusions:(const std::vector<FloatExclusion> &)exclusionsPt
{
  NSTextContainer *textContainer = [[NSTextContainer alloc] initWithSize:size];

  if (!exclusionsPt.empty()) {
    NSMutableArray<UIBezierPath *> *paths = [NSMutableArray arrayWithCapacity:exclusionsPt.size()];
    for (const auto &exclusionPt : exclusionsPt) {
      CGFloat heightPt = exclusionPt.blockEnd - exclusionPt.blockStart;
      if (heightPt <= 0) {
        continue;
      }
      if (exclusionPt.leftInset > 0) {
        [paths addObject:[UIBezierPath bezierPathWithRect:CGRectMake(
                                                              0,
                                                              exclusionPt.blockStart,
                                                              exclusionPt.leftInset,
                                                              heightPt)]];
      }
      // A right float is measured from the container's own right edge, so an
      // unbounded width (`nowrap`) has no edge to hang it off and it is
      // dropped rather than placed at infinity.
      if (exclusionPt.rightInset > 0 && !isinf(size.width)) {
        [paths addObject:[UIBezierPath bezierPathWithRect:CGRectMake(
                                                              size.width - exclusionPt.rightInset,
                                                              exclusionPt.blockStart,
                                                              exclusionPt.rightInset,
                                                              heightPt)]];
      }
    }
    if (paths.count > 0) {
