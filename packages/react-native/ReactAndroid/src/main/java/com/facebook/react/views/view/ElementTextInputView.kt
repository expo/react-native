...
internal open class ElementTextInputView(context: Context) : AppCompatEditText(context) {
  ...
  /**
   * The platform's chrome is the user-agent's, and an author's box replaces it.
   *
   * A browser draws an input's border from its UA sheet and stops the moment the
   * author states a border or a background of their own — `border: none` on a
   * search field is not a request for the platform's rule *as well*. Ours drew
   * both: a composer styled as a filled, rounded pill wore the Material
   * underline through it.
   *
   * The signal is the composite background React Native builds for this view. It
   * grows a `border` layer the first time a border width is stated — including a
   * zero, which is how a design says "no rule" — and a `background` layer the
   * first time a fill is. Either one means the box is the author's now.
   */
  ...
  private fun applyInputType(
    ...
    var inputTypeFlags =
        when (key) {
          /*
           * DOM-CSS-LIMITATION(android-no-spellcheck-on-email-or-url)
           *
           * `isSuggestionsEnabled()` returns false for every variation but the plain text ones, so
           * choosing these keyboards also turns the spell checker off — while HTML lists both
           * Email and URL among the types a user agent should consider checkable. The keyboard is
           * the more visible half of the attribute, so it wins.
           */
    ...
    if (secure) {
       ...
       *
       * HTML keeps `spellcheck` and `autocorrect` apart — one marks mistakes, the other rewrites
       * them (§6.8.5 vs §6.8.8) — and iOS has a trait for each, so it can say "underline but do not
       * rewrite". Android has one flag for both: `TextView.isSuggestionsEnabled()` returns false as
       * soon as `TYPE_TEXT_FLAG_NO_SUGGESTIONS` is set, and that single predicate gates the spell
       * checker AND the IME's suggestion strip (AOSP TextView.java). There is no
       * check-but-do-not-correct flag to reach for.
       *
       * So `spellcheck="false"` is honoured, and takes autocorrection with it. That direction is
       * chosen deliberately: the author asked for something to be off, and turning off more than
       * they asked is a smaller betrayal than leaving on the thing they named.
       */
      if (!spellCheck) {
        inputTypeFlags =
    ...
    /*
     * Applied only when it actually CHANGED, which is not an optimisation.
     *
     * `TextView.setInputType` has no early-out for an unchanged type, and it ends with
     * `imm.restartInput(this)` — unconditionally, every call (AOSP TextView.java). Restarting the
     * input connection is how the platform says "forget everything about this editor", and what
     * gets forgotten is the IME's COMPOSING REGION. [commitProps] runs on every prop update, so
     * applying this unconditionally restarted the IME on every keystroke of a controlled field:
     * the half-typed word a latin keyboard is predicting from, and on a CJK keyboard the candidate
     * session — the very thing the `markedTextRange` refusal protects on iOS, undone from the
     * other side.
     *
     * Tracked in a field rather than compared against `inputType`, because the first application
     * has to happen even when the computed flags match the widget's default — and they do, for a
     * plain `<input type="text">`. Comparing against the live value would skip the initial
     * `isSingleLine` below and leave a single-line field laying its hint out over several lines.
     *
     * `applyEnterKeyHint` needs no such guard: `setImeOptions` is a plain field write.
     */
