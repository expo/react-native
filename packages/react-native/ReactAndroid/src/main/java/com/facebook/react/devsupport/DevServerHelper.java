...
public class DevServerHelper {
    ...
    public final DeveloperSettings mSettings;
    ...
    public DevServerHelper(DeveloperSettings developerSettings, Context applicationContext, PackagerConnectionSettings packagerConnectionSettings) {
    ...
    public void openPackagerConnection(final String clientId, final PackagerCommandListener commandListener) {
        ...
        new AsyncTask<Void, Void, JSPackagerClient>() {
            ...
            protected JSPackagerClient doInBackground(Void... backgroundParams) {
                ...
                JSPackagerClient packagerClient = new JSPackagerClient(clientId, mPackagerConnectionSettings, handlers, onPackagerConnectedCallback);
                packagerClient.init();
    ...
    public void openInspectorConnection() {
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                Map<String, String> metadata = AndroidInfoHelpers.getInspectorHostMetadata(mApplicationContext);
                mInspectorPackagerConnection = new CxxInspectorPackagerConnection(getInspectorDeviceUrl(), metadata.get("deviceName"), mPackageName);
    ...
    public String getWebsocketProxyURL() {
        return String.format(Locale.US, "ws://%s/debugger-proxy?role=client", mPackagerConnectionSettings.getDebugServerHost());
    }
    ...
    private String getInspectorDeviceId() {
        ...
        // [Source: Android docs]
        String androidId = Secure.getString(mApplicationContext.getContentResolver(), Secure.ANDROID_ID);
        String rawDeviceId = String.format(Locale.US, "android-%s-%s-%s", packageName, androidId, InspectorFlags.getFuseboxEnabled() ? "fusebox" : "legacy");
        return getSHA256(rawDeviceId);
    }

    private String getInspectorDeviceUrl() {
        return String.format(Locale.US, "http://%s/inspector/device?name=%s&app=%s&device=%s&profiling=%b", mPackagerConnectionSettings.getDebugServerHost(), Uri.encode(AndroidInfoHelpers.getFriendlyDeviceName()), Uri.encode(mPackageName), Uri.encode(getInspectorDeviceId()), InspectorFlags.getIsProfilingBuild());
    }
   ...
   */
    ...
    private String createBundleURL(String mainModuleID, BundleType type, String host) {
        try {
            return (String) Class.forName("host.exp.exponent.ReactNativeStaticHelpers").getMethod("getBundleUrlForActivityId", int.class, String.class, String.class, String.class, boolean.class, boolean.class).invoke(null, mSettings.getExponentActivityId(), host, mainModuleID, type.typeID(), getDevMode(), getJSMinifyMode());
    ...
    public String getDevServerBundleURL(final String jsModulePath) {
        return createBundleURL(jsModulePath, BundleType.BUNDLE, mPackagerConnectionSettings.getDebugServerHost());
    }
    ...
    private String createLaunchJSDevtoolsCommandUrl() {
        return String.format(Locale.US, "http://%s/launch-js-devtools", mPackagerConnectionSettings.getDebugServerHost());
    }
   ...
   */
    ...
    public File downloadBundleResourceFromUrlSync(final String resourcePath, final File outputFile) {
        final String resourceURL = createResourceURL(mPackagerConnectionSettings.getDebugServerHost(), resourcePath);
    ...
    public void openDebugger(@Nullable final ReactContext context, final String errorMessage) {
        ...
        String requestUrl = String.format(Locale.US, "http://%s/open-debugger?device=%s", mPackagerConnectionSettings.getDebugServerHost(), Uri.encode(getInspectorDeviceId()));
        Request request = new Request.Builder().url(requestUrl).method("POST", RequestBody.create(null, "")).build();
        mClient.newCall(request).enqueue(new Callback() {
