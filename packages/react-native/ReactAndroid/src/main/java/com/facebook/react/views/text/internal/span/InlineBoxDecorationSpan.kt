 ...
 * §10.6.1 requires, and `PreparedLayoutTextView.onDraw` admits exactly that
 * much overflow past its clip using `blockAxisOverflow` below. Measured on
 * device for a box computed as 361.8x70px: border 362x70 (y=2007..2076),
 * outline 377x84 — the outline sitting `outlineOffset + outlineWidth` outside
 * the border box in both axes, as it should.
 */
internal class InlineBoxDecorationSpan(
  ...
  /**
   * How far this box paints outside the line box, in pixels.
   *
   * An inline box never grows the line box (CSS2 §10.6.1) — its block-axis
   * padding, border and outline simply overflow it — so a view whose drawing
   * surface is the measured text clips them away unless it makes room for
   * exactly this much. The mirror of `AttributedString::inlineBoxBlockAxisOverflow`
   * in C++, which iOS uses for the same purpose.
   */
  val blockAxisOverflow: Float
    get() {
      val outline = if (outlineWidth > 0f) outlineOffset + outlineWidth else 0f
      val top = paddingTop + borderTopWidth + outline
      val bottom = paddingBottom + borderBottomWidth + outline
      return if (top > bottom) top else bottom
    }
