...
public class NetworkingModule extends NativeNetworkingAndroidSpec {
   ...
   */
    ...
    public static String TAG = NativeNetworkingAndroidSpec.NAME;
    ...
    @Nullable
    public static com.facebook.react.modules.network.CustomClientBuilder customClientBuilder = null;

    public final OkHttpClient mClient;

    public final ForwardingCookieHandler mCookieHandler = new ForwardingCookieHandler();
    ...
    public final Set<Integer> mRequestIds = new HashSet<>();
    ...
    public NetworkingModule(ReactApplicationContext reactContext, @Nullable String defaultUserAgent, OkHttpClient client, @Nullable List<NetworkInterceptorCreator> networkInterceptorCreators) {
        super(reactContext);
   ...
   */
    ...
    public void sendRequest(String method, String url, double requestIdAsDouble, ReadableArray headers, ReadableMap data, String responseType, boolean useIncrementalUpdates, double timeoutAsDouble, boolean withCredentials) {
        ...
        try {
            ...
            ResponseUtil.onRequestError(getReactApplicationContextIfActiveOrWarn(), requestId, th.getMessage(), th);
        }
    }
   ...
   */
    public void sendRequestInternal(String method, String url, final int requestId, ReadableArray headers, ReadableMap data, final String responseType, final boolean useIncrementalUpdates, int timeout, boolean withCredentials) {
            ...
            clientBuilder.addNetworkInterceptor(chain -> {
                ...
                ProgressResponseBody responseBody = new ProgressResponseBody(originalResponse.body(), new ProgressListener() {
                    ...
                    public void onProgress(long bytesWritten, long contentLength, boolean done) {
                        ...
                        ResponseUtil.onDataReceivedProgress(reactApplicationContext, requestId, bytesWritten, contentLength);
                        last = now;
                    }
                });
        ...
        client.newCall(requestBuilder.build()).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                ResponseUtil.onResponseReceived(reactApplicationContext, requestId, response.code(), translateHeaders(response.headers()), response.request().url().toString());
                try {
    ...
    private RequestBody wrapRequestBodyWithProgressEmitter(final RequestBody requestBody, final int requestId) {
        if (requestBody == null) {
            return null;
        }
        final ReactApplicationContext reactApplicationContext = getReactApplicationContextIfActiveOrWarn();
        return RequestBodyUtil.createProgressRequest(requestBody, new ProgressListener() {
            ...
            public void onProgress(long bytesWritten, long contentLength, boolean done) {
                long now = System.nanoTime();
                if (done || shouldDispatch(now, last)) {
                    ResponseUtil.onDataSend(reactApplicationContext, requestId, bytesWritten, contentLength);
                    last = now;
                }
            }
        });
