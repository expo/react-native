...
public abstract class DevSupportManagerBase implements DevSupportManager {
    ...
    public final PausedInDebuggerOverlayManager mPausedInDebuggerOverlayManager;
    ...
    public DevSupportManagerBase(Context applicationContext, ReactInstanceDevHelper reactInstanceDevHelper, @Nullable String packagerPathForJSBundleName, boolean enableOnCreate, @Nullable RedBoxHandler redBoxHandler, @Nullable DevBundleDownloadListener devBundleDownloadListener, int minNumShakes, @Nullable Map<String, RequestHandler> customPackagerCommandHandlers, @Nullable SurfaceDelegateFactory surfaceDelegateFactory, @Nullable DevLoadingViewManager devLoadingViewManager, @Nullable PausedInDebuggerOverlayManager pausedInDebuggerOverlayManager) {
        ...
        mDevServerHelper = new DevServerHelper(mDevSettings, mApplicationContext, mDevSettings.getPackagerConnectionSettings());
        ...
        mPausedInDebuggerOverlayManager = pausedInDebuggerOverlayManager != null ? pausedInDebuggerOverlayManager : new PausedInDebuggerOverlayDialogManager(() -> {
    ...
    private void logJSException(Exception e) {
        ...
        if (e instanceof JavascriptException) {
            ...
            showNewError(e.getMessage().toString(), new StackFrame[] {}, JSEXCEPTION_ERROR_COOKIE, ErrorType.JS);
        } else {
   ...
   */
    ...
    private int getExponentActivityId() {
        return mDevServerHelper.mSettings.getExponentActivityId();
    }
    ...
    public void showDevOptionsDialog() {
        ...
        if (mDevSettings.isDeviceDebugEnabled()) {
            ...
            String debuggerItemString = mApplicationContext.getString(isConnected ? R.string.catalyst_debug_open : R.string.catalyst_debug_open_disabled);
            if (!isConnected) {
                disabledItemKeys.add(debuggerItemString);
            }
            options.put(debuggerItemString, () -> openDebugger());
        }
        options.put(mApplicationContext.getString(R.string.catalyst_change_bundle_location), () -> {
            Activity context = mReactInstanceDevHelper.getCurrentActivity();
            if (context == null || context.isFinishing()) {
        ...
        options.put(mApplicationContext.getString(R.string.catalyst_inspector_toggle), new DevOptionHandler() {
        ...
        options.put(mDevSettings.isHotModuleReplacementEnabled() ? mApplicationContext.getString(R.string.catalyst_hot_reloading_stop) : mApplicationContext.getString(R.string.catalyst_hot_reloading), () -> {
        ...
        options.put(mDevSettings.isFpsDebugEnabled() ? mApplicationContext.getString(R.string.catalyst_perf_monitor_stop) : mApplicationContext.getString(R.string.catalyst_perf_monitor), () -> {
        ...
        if (jsExecutorDescription != null) {
            ...
            jsExecutorLabel.setText(context.getString(R.string.catalyst_dev_menu_sub_header, jsExecutorDescription));
        ...
        ListAdapter adapter = new ArrayAdapter<String>(context, android.R.layout.simple_list_item_1, options.keySet().toArray(new String[0])) {
        ...
        mDevOptionsDialog = new AlertDialog.Builder(context).setCustomTitle(header).setAdapter(adapter, (dialog, which) -> {
            optionHandlers[which].onOptionSelected();
            mDevOptionsDialog = null;
        }).setOnCancelListener(dialog -> mDevOptionsDialog = null).create();
        mDevOptionsDialog.show();
        if (mCurrentReactContext != null) {
            mCurrentReactContext.getJSModule(RCTNativeAppEventEmitter.class).emit("RCTDevMenuShown", null);
        }
    }
   ...
   */
    ...
    private void resetCurrentContext(@Nullable ReactContext reactContext) {
        ...
        if (mCurrentReactContext != null) {
            try {
                ...
                mCurrentReactContext.getJSModule(HMRClient.class).setup("android", path, host, port, mDevSettings.isHotModuleReplacementEnabled(), scheme);
    ...
    private void showDevLoadingViewForUrl(String bundleUrl) {
        ...
        mDevLoadingViewManager.showMessage(mApplicationContext.getString(R.string.catalyst_loading_from_url, parsedURL.getHost() + ":" + port));
    ...
    protected void showDevLoadingViewForRemoteJSEnabled() {
        ...
        mDevLoadingViewManager.showMessage(mApplicationContext.getString(R.string.catalyst_debug_connecting));
        mDevLoadingViewVisible = true;
    }
   ...
   */
    private void compatRegisterReceiver(Context context, BroadcastReceiver receiver, IntentFilter filter, boolean exported) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE && context.getApplicationInfo().targetSdkVersion >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
    ...
    public void openDebugger() {
        mDevServerHelper.openDebugger(mCurrentReactContext, mApplicationContext.getString(R.string.catalyst_open_debugger_error));
    }

    @Override
    public void showPausedInDebuggerOverlay(String message, PausedInDebuggerOverlayCommandListener listener) {
