   ...
   */
    public static Map<String, String> getInspectorHostMetadata(@Nullable Context applicationContext) {
        ...
        if (applicationContext != null) {
            ...
            appDisplayName = labelResourceId == 0 ? applicationInfo.nonLocalizedLabel.toString() : applicationContext.getString(labelResourceId);
        }
        return MapBuilder.<String, String>of("appDisplayName", appDisplayName, "appIdentifier", appIdentifier, "platform", "android", "deviceName", Build.MODEL, "reactNativeVersion", getReactNativeVersionString());
    }

    private static String getReactNativeVersionString() {
        Map<String, Object> version = ReactNativeVersion.VERSION;
        return version.get("major") + "." + version.get("minor") + "." + version.get("patch") + (version.get("prerelease") != null ? "-" + version.get("prerelease") : "");
    }
