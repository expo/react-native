 ...
 *
 * `contentAnchor` cancels a movement of the CONTENT: it remembers where a
 * visible row was before a mounting transaction, finds it after, and moves the
 * offset by however far it travelled. Whether it is ALLOWED to is a separate
 * question, and one that was answered wrongly for a long time — the correction
 * was computed and then discarded whenever the scroll view happened to be
 * decelerating, which is most of when it matters.
 *
 * Counting both halves is what makes that visible. `wanted` without `applied`
 * is content moving under the reader with nothing cancelling it, and the
 * difference is the number of points they saw jump.
 */
typedef struct {
  uint64_t wanted;
  uint64_t applied;
