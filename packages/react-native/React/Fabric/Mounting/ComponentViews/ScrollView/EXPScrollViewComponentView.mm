 ...
 * one it moves on its own — composing an inset, undoing a `setContentSize:`
 * clamp — is deliberately not, because the reader did not move. The sum of the
...
{
  return CGPointMake(
      _scrollView.contentOffset.x - _reportedOffset.x, _scrollView.contentOffset.y - _reportedOffset.y);
}
 ...
 * news of the same kind — and it changes without an inset changing: a clamp
 * repair moves the offset behind the tree's back. Emitting only on a CHANGE is
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  // The SHIFT it leaves behind is news, though — that is the one thing a caller
