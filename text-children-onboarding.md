...
3. **Inheritance** is defined on the *element tree*, per
   ...
   properties (`color`, `fontSize`, `fontFamily`, `fontWeight`, `fontStyle`, `fontVariant`,
   `letterSpacing`, `lineHeight`, `textAlign`, `textTransform`, `whiteSpace`)
   cascade from elements into bare-text content. A root `<Text>` does **not** inherit unless
   it opts in via `style={{all: 'unset'}}` — the pixel-identity exception, expressed through
   the standard `all` property (text-inheritance-boundaries.md).
...
8. **White-space**: collapsed per
   [css-text-3 §3](https://www.w3.org/TR/css-text-3/#white-space-property), inside the new
   anonymous IFCs only — authored `<Text>` keeps its verbatim newlines, so no back-compat is
   disturbed. `normal`, `nowrap`, `pre`, `pre-wrap`, `pre-line` and `break-spaces` all parse.
   **`break-spaces` behaves as `pre-wrap`**: it differs only in whether a run of preserved
   spaces at a wrap point is measured or allowed to hang, and both platform engines hang it
   with no knob to say otherwise. Identical unless a space run outruns the line.
9. **Deliberate deviations** (documented, not bugs): `break-spaces` hangs rather than
   measures (see 8), and **`box-decoration-break: clone` is not implemented** — only the
   initial `slice`. Both are the same wall: an inline box's edges are reserved *before* the
   engine breaks lines and the breaks depend on that reservation, so resolving either needs
   to be inside the line breaker. TextKit and `StaticLayout` both hand back finished lines.
   Painting `clone`'s per-line edges without reserving advance for them would overlap glyphs,
   which is worse than not offering the value. The RN `Image`
