...
> **Reader's note (2026-08-13).** This file is a dated task log; entries describe
> the state at their timestamps. Since then the element catalog moved OUT of
> react-native into the provider package (`packages/expo-intrinsics`), `div`'s
> bespoke shadow node was deleted (block elements ride the generic
> `element-box`), the `unknown` component was renamed `inline-text`, and Text's
> inheritance opt-in became the standard `all` property. Current design docs:
> `packages/expo-intrinsics/__docs__/ProvidingHTMLElementsInTheFramework.md`,
> `text-inheritance-boundaries.md`, `string-children-perf-plan.md`.
