...
- **Compatibility split (resolves a real contradiction with the `<Text>` exception):**
  ...
  `<Text>` is an explicit opt-in — now spelled `<Text style={{all: 'unset'}}>` per
  element (the standard property replaced the original `inheritViewTextStyles` prop;
  text-inheritance-boundaries.md): root Text ships `all: 'initial'` in its user-agent
  style, and `ParagraphShadowNode::getContent` seeds from the cascade whenever the
  InheritanceBoundary trait is absent.
...
4. **Inheritance** with the root-`<Text>` boundary compatibility split (§3.D; the
   opt-in is `all: 'unset'` since the `all`-property rework). Accept:
...
Status: **prototyped, demoted, and deleted** (2026-08-16). It lived at
`ReactCommon/react/renderer/uimanager/AnonymousTextCommitHook.{h,cpp}` (~120 lines) with a
registration in `UIManager::setComponentDescriptorRegistry`. The registration went when the
anonymous-box design won; the files stayed behind, compiled but instantiated by nothing, and
still referring to the since-deleted `RawText` — close enough to the real thing that a
reader could mistake it for the implementation. Git has it if it is ever wanted again.

What it taught us, which is the part worth keeping: the first run trips the single-parent
assert (`ShadowNodeFamily::setParent`) when adopting React-owned text children. That is
resolvable — last-writer-wins, or a scoped reassignment API — but it is emblematic of the
approach's flaw: synthesized boxes leak into the "DOM". It also needs DevTools/DOM
attribution work and identity caching. The anonymous-box design has none of those problems
because its boxes never enter the shadow tree at all.
