...
w(' */');
w('');
w("import type {RNTesterModule} from '../../types/RNTesterTypes';");
w('');
w("import * as React from 'react';");
w("import {ScrollView, Text, View} from 'react-native';");
w('');
...
w('  const all: {[string]: CaseEntry} = globalThis.__gridConformance ?? {};');
w('  const entry: CaseEntry = all[caseId] ?? {items: {}};');
w("  if (key === 'container') {");
...
w('        style={[containerStyle, {backgroundColor: CONTAINER_TINT}]}>');
w('        {items.map((item, i) => (');
w('          <View');
w('            key={i}');
w('            collapsable={false}');
...
w('            }}');
w('            /* $FlowExpectedError[incompatible-type] grid style keys */');
w('            style={[item.style, {backgroundColor: ITEM_TINT}]}>');
